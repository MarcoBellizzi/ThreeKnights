...
import logic.characters_jan.ArcherThread;
import logic.characters_jan.AttackThread;
import logic.characters_jan.GoblinThread;
import logic.characters_jan.JumpThread;
import logic.characters_jan.Knight;
import logic.characters_jan.KnightMoveThread;
import logic_jan.LifePoints;
import logic_jan.Mondo;
...
public class MyLevelPanel extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		for(Archer a: at.archers) {
			a.drawArcher(g, this);                    //commentato
		}
				...
				if(!displayingHighscore) {
					...
					if(knight.getCoins() == 18) {
						...
						try {
							...
							while(highscoreTimes[i] >= getTime() && i >= 0) {
								position = i--;
							}
